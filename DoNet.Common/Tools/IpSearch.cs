...
namespace DoNet.Common.Tool
{
    ...
    public class IpSearch
    {
        ...
        PHCZIP pcz = new PHCZIP();

        string filePath = "";

        bool fileIsExsit = true;

        /// <summary>
        /// 判断IP归属地类
        /// </summary>
        /// <param name="ipDataPath">IP数据文件路径</param>
        public IpSearch(string ipDataPath)
        {
            ipDataPath=DoNet.Common.IO.PathMg.CheckPath(ipDataPath);
            if (!File.Exists(ipDataPath))
            {
                throw new Exception(ipDataPath + "　文件不存在！");
            }
            filePath = ipDataPath;
            if (fileIsExsit)
            {
